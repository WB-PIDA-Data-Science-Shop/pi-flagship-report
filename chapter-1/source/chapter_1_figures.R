...
# open data
BOOST <- read_csv("chapter-1/data/output/boost_clean.csv", show_col_types = F)
GTMI <- read_csv("chapter-1/data/output/gtmiwide_clean.csv", show_col_types = F)
...
ggsave(
  here("chapter-1", "figs", "fig_1_2.png"),
  height = 8,
  width = 12,
  bg = "white"
)
...
ggsave(
  plot = last_plot(),
  filename = "chapter-1/figs/fig_1_3.png",
  width = 12,
  height = 9,
  bg = "white"
)
...
leftpanel <- GTMI %>%
  ...
  mutate(
    ...
    incomegroup = case_when(
      ...
      incomegroup ==
        "Upper middle income" ~ "Upper middle income (52 countries)",
      incomegroup ==
        "Lower middle income" ~ "Lower middle income (54 countries)",
      incomegroup == "Low income" ~ "Low income (26 countries)"
    )
  ) %>%
  ggplot(aes(
    y = reorder(variable, -variable_order),
    x = value,
  ...
  labs(
    ...
    x = "Number of countries in income group",
...
ggsave(
  here("chapter-1", "figs", "fig_1_6.png"),
  height = 8,
  width = 12,
  bg = "white"
)
...
GTMItrans2022 <- GTMI %>%
  filter(year == "2022") %>%
  mutate(
    fmis_trans = case_when(
      fmis == 0 ~ "No system",
      fmis == 1 &
        fmis_gov != 2 ~ "System present, but country does not publish",
    ...
    customs_trans = case_when(
      customs == 0 ~ "No system",
      customs == 1 &
        customs_gov != 2 ~ "System present, but country does not publish",
      customs == 1 &
        customs_gov == 2 ~ "System present and country does publish"
    ),
    hrmis_trans = case_when(
      hrmis == 0 ~ "No system",
      hrmis == 1 &
        hrmis_gov != 2 ~ "System present, but country does not publish",
      hrmis == 1 & hrmis_gov == 2 ~ "System present and country does publish"
    ),
    payroll_trans = case_when(
      payroll == 0 ~ "No system",
      payroll == 1 &
        payroll_gov != 2 ~ "System present, but country does not publish",
      payroll == 1 &
        payroll_gov == 2 ~ "System present and country does publish"
    ),
    proc_trans = case_when(
      proc == 0 ~ "No system",
      proc == 1 &
        proc_gov != 2 ~ "System present, but country does not publish",
...
# plot figure 1.7
GTMItrans2022 %>%
  count(system, trans, trans_order) %>%
  mutate(
    trans = factor(
      trans,
      levels = c(
        "System present, but country does not publish",
        "System present and country does publish",
        "No system"
      ) |> 
        rev()
    )
  ) %>%
  ggplot(aes(x = system, y = n, fill = trans)) +
  geom_bar(stat = "identity") +
  scale_fill_manual(
    name = "Transparency\nin system governance",
    values = c(
      "No system"                                        = egvpi_colors[2],
      "System present, but country does not publish"     = egvpi_colors[1],
      "System present and country does publish"          = egvpi_colors[6]
    )
  ) +
  labs(
...
ggsave(
  filename = "chapter-1/figs/fig_1_7.png",
  width = 12,
  height = 9,
  bg = "white"
)
